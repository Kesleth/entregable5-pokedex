  ...
  return (
    <section>
      ...
      <section className="px-2 py-16">
        <article className={`max-w-[768px] mx-auto shadow-xl p-2 border-b-2 ${bordersByType[pokemon?.types[0].type.name]} shadow-lg ${shadowByType[pokemon?.types[0].type.name]} rounded-2xl border-r-2 `}>
          {/* seccion superior*/}

          <section
            className={`rounded-2 bg-gradient-to-b  ${backgraundByType[pokemon?.types[0].type.name]}  relative h-[150px]`}
          >
          ...
          <section>
            ...
            <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2">
              <hr />
              <h2 className={`capitalize text-4xl ${textByType[pokemon?.types[0].type.name]}}  font-semibold  `}>
                {pokemon?.name}{""}
              </h2>
              <hr />
            </div>
            ...
            <div className="grid grid-cols-2 gap-6 ">
              <div className="grid grid-rows-2 p-2 justify-items-center">
                ...
                <span className={`font-bold ${textByType[pokemon?.types[0].type.name]}  `}>{pokemon?.weight}</span>
              </div>
              <div className="grid grid-rows-2 p-2 justify-items-center">
                <h5 className="font-semibold">heigth</h5>
                <span className={`font-bold ${textByType[pokemon?.types[0].type.name]}`}>{pokemon?.height}</span>
              </div>
            </div>
            ...
            <section className=" grid md:grid-cols-2 gap-4 ">
              ...
              <section className="text-center">
                <h3 className={` rounded-xl text-2xl font-semibold bg-gradient-to-b ${backgraundByType[pokemon?.types[0].type.name]}`}>Types</h3>

                <section className="grid grid-cols-2 gap-4 mt-4">
                  {pokemon?.types.map((type) => (
                    <article
                      className={`text-black border-[1px] border-gray-300 rounded-r-lg transition transform hover: -translate-y-2 font-semibold p-2 px-8 text-center capitalize truncate ${bordersByType[pokemon?.types[0].type.name]}`} key={type.type.name}
                    >
                      {type.type.name}
                    </article>
                  ))}
              ...
              <section className="text-center font-semibold ">
                ...
                <section className=" grid grid-cols-2 gap-4 mt-4">
                  {pokemon?.abilities.map((ability) => (
                    <article
                      className={`border-[1px] border-gray-300 rounded-r-lg transition transform hover: -translate-y-2 font-semibold p-2 px-8 text-center capitalize truncate ${bordersByType[pokemon?.types[0].type.name]} `}
